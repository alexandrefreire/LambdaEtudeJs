...
QUnit.skip("productsOrderedByPriceAscending", function ( assert) {
  var orderedProducts = this.finder.productsOrderedByPriceAscending()

  assert.equal(orderedProducts.length, 5)
  assert.deepEqual(orderedProducts, [ fireTruck, baseball, frisbee, barbieClassic, toyConvertible ])
})

QUnit.skip("productsOrderedByPriceDescending", function ( assert) {
  var orderedProducts = this.finder.productsOrderedByPriceDescending()

  assert.equal(orderedProducts.length, 5)
  assert.deepEqual(orderedProducts, [ toyConvertible, barbieClassic, frisbee, fireTruck, baseball ])
})
